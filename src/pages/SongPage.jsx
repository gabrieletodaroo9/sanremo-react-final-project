...
export default function SongPage() {
  ...
  return (
    <div className="container mt-5 pt-4 pt-md-0 pb-5">
      ...
      <button onClick={() => navigate(-1)} className="btn fs-3 border-0 mb-5 p-0 text-muted"><i className="bi bi-arrow-left"></i> Torna</button>
            ...
            <div className="mb-4">
              ...
              {song.artists.map((artist, index) => (
                <Link key={artist.id} to={`/artists/${artist.id}`} className="text-decoration-none">
                  <h3 className="text-first fw-bold mb-0 d-inline">{artist.name}{index < song.artists.length - 1 && " & "}</h3>
                </Link>
              ))}

              <p className="text-muted small text-uppercase mt-2">Sanremo {song.edition.year} — {song.edition.location}</p>
            </div>
        ...
        <div className="col-md-7 ps-md-5">
          {song.youtube_url && (
            <div className="ratio ratio-16x9 mb-4 mb-md-5 shadow-sm rounded bg-dark overflow-hidden">
              <iframe src={`https://www.youtube.com/embed/${song.youtube_url.split('v=')[1]}`} title="YouTube" allowFullScreen></iframe>
            </div>
          )}
          <h3 className="fw-bold text-uppercase mb-md-4 text-first tracking-widest ps-0 ps-md-4">Testo</h3>
          <div className="lyrics-display ps-0 ps-md-4 rounded bg-main" style={{ whiteSpace: "pre-line", fontSize: "1.15rem", lineHeight: "1.9", color: "#333" }}>
